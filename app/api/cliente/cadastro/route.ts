...
export async function POST(req: Request) {
  ...
  // O e-mail é a identidade do cliente: se já existe, mandamos entrar em vez
  // de criar outro (e assim ninguém sobrescreve o cadastro de outra pessoa).
  const [existente] = await db.select().from(clientes).where(eq(clientes.email, email));
  if (existente) {
    return NextResponse.json(
      { error: "Este e-mail já tem cadastro. Faça login ou use 'Esqueci minha senha'." },
      { status: 409 }
    );
  }
  ...
  try {
    ...
    // Dois cadastros com o mesmo e-mail cravados no mesmo instante: quem
    // chega depois esbarra no "único" do banco. Vira o mesmo recado da
    // checagem acima, e não um erro sem explicação.
    if (emailJaExiste(e)) {
      return NextResponse.json(
        { error: "Este e-mail já tem cadastro. Faça login ou use 'Esqueci minha senha'." },
        { status: 409 }
